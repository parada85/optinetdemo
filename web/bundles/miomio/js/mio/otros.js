	...
	$("#contarproductofam2").text(parseInt($("#contarproductofam2").text())-1);
			...
			$('.select_filter').wijdropdown();
				...
				$('#guardar').live('click', function () {
						...
						if (contador == 0  || isNaN($('.entregado input').val()) || parseFloat($('.entregado input').val()) < parseFloat($('#totalventa').html()))
        ...
        window.location = Routing.generate('cambiarlocale',{idioma: 'es'});
				...
				$('#guardarreserva').live('click', function () {
						...
						var contador = tabla4.fnSettings().fnRecordsTotal();
						var cantidad = parseInt($('#totalventa').html());

						if (isNaN(input) || contador == 0 || input == "" || input < 0 || input > cantidad || parseInt($('.entregado input').val()) < input)
							$("#error").wijdialog('open');
						else
				...
				$('#guardarapartado').live('click', function () {
						...
						if (isNaN(input) || contador == 0 || input == "" || input < 0 || input > cantidad || input > parseInt($(".entregado input").val()) )
							$("#error").wijdialog('open');
						else
